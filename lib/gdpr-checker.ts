...
export class GDPRCookieChecker {
  ...
  async checkWebsite(url: string): Promise<GDPRCheckResult> {
    ...
    try {
      ...
      // Analyze cookies
      result.cookies_found = this.analyzeCookies(
  ...
  private checkCookieBanner(html: string): boolean {
    ...
    if (
      cookieBannerSelectors.some((selector) => htmlLower.includes(selector))
    ) {
      return true;
    }
  ...
  private checkGranularConsent(html: string): boolean {
    ...
    const hasToggle =
      htmlLower.includes("toggle") || htmlLower.includes("switch");

    if (hasCheckboxes || hasToggle) {
      return granularIndicators.some((indicator) =>
  ...
  private checkCookiePolicy(
    ...
    const found =
      ...
      htmlLower.includes("cookie information");
  ...
  private calculateComplianceScore(result: GDPRCheckResult): void {
    ...
    if (result.cookies_found.length > 0) {
      const nonEssentialCookies = result.cookies_found.filter(
        (c) => c.category !== "essential"
      );
      if (nonEssentialCookies.length > 0 && !result.has_consent_mechanism) {
        score -= 10;
        result.issues.push(
          "Non-essential cookies found without proper consent mechanism"
        );
      } else {
        score += 5;
      }
  ...
  private generateRecommendations(result: GDPRCheckResult): void {
    ...
    if (!result.has_consent_mechanism) {
      result.recommendations.push(
        "Add proper consent mechanism (Accept/Reject buttons)"
      );
    }

    if (!result.has_reject_all_option) {
      result.recommendations.push(
        "Provide an easy way for users to reject all non-essential cookies"
      );
    }

    if (!result.has_granular_consent) {
      result.recommendations.push(
        "Allow users to choose specific cookie categories"
      );
    }

    if (!result.has_privacy_policy) {
      result.recommendations.push(
        "Create and link to a comprehensive privacy policy"
      );
    }

    if (!result.has_cookie_policy) {
      result.recommendations.push(
        "Create a detailed cookie policy explaining cookie usage"
      );
    }
    ...
    if (insecureCookies.length > 0) {
      result.recommendations.push(
        "Set Secure flag on all cookies containing sensitive data"
      );
    }
  }
}
