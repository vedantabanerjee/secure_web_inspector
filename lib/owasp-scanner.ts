...
export class OWASPTop10Scanner {
  ...
  private async testBrokenAccessControl(url: string): Promise<void> {
    ...
    const adminPaths = ["/admin", "/administrator", "/admin.php", "/wp-admin", "/.env", "/.git/config"]
  ...
  private async testCryptographicFailures(url: string): Promise<void> {
    ...
    if (response) {
      ...
      const sensitivePatterns = [/password=[\w]+/i, /token=[a-zA-Z0-9]+/i, /key=[a-zA-Z0-9]+/i, /api_key=[a-zA-Z0-9]+/i]
  ...
  private async testInjectionVulnerabilities(url: string): Promise<void> {
    ...
    if (html.includes("<script>") && !response.headers.get("content-security-policy")) {
      this.vulnerabilities.push({
        id: `A03-${Date.now()}`,
        category: "A03:2021 – Injection",
        severity: "High",
  ...
  private detectSQLError(responseText: string): boolean {
    ...
    return sqlErrors.some((error) => responseText.toLowerCase().includes(error))
  }
  ...
  private async testInsecureDesign(url: string): Promise<void> {
    ...
    for (const [header, description] of Object.entries(securityHeaders)) {
      if (!response.headers.get(header.toLowerCase())) {
        const severity = ["Content-Security-Policy", "X-Frame-Options"].includes(header) ? "High" : "Medium"
        this.vulnerabilities.push({
          id: `A04-${Date.now()}`,
  ...
  private async testSecurityMisconfiguration(url: string): Promise<void> {
    ...
    if (response.status >= 400) {
      ...
      if (errorIndicators.some((indicator) => html.toLowerCase().includes(indicator))) {
        this.vulnerabilities.push({
          id: `A05-${Date.now()}`,
          category: "A05:2021 – Security Misconfiguration",
          severity: "Medium",
  ...
  private async testAuthenticationFailures(url: string): Promise<void> {
    ...
    if (html.toLowerCase().includes('type="password"')) {
      const passwordRequirements = ["minimum", "uppercase", "lowercase", "special character", "digit", "length"]

      if (!passwordRequirements.some((req) => html.toLowerCase().includes(req))) {
        this.vulnerabilities.push({
          id: `A07-${Date.now()}`,
          category: "A07:2021 – Identification and Authentication Failures",
          severity: "Medium",
  ...
  private async testIntegrityFailures(url: string): Promise<void> {
    ...
    const linkMatches = html.match(/<link[^>]+href=["']https?:\/\/[^"']+["'][^>]*>/gi) || []
    ...
    if (externalResourcesWithoutSRI.length > 0) {
      this.vulnerabilities.push({
        ...
        recommendation: "Implement Subresource Integrity (SRI) for external resources",
        cwe_id: "CWE-345",
      })
    }
  }
  ...
  private async testLoggingFailures(url: string): Promise<void> {
    ...
    if (responses.length === 5 && responses.every((status) => status === 200)) {
      this.vulnerabilities.push({
        ...
        description: "Multiple rapid requests all succeeded without rate limiting",
  ...
  private async testSSRFVulnerabilities(url: string): Promise<void> {
    ...
    const ssrfPayloads = ["http://169.254.169.254/", "http://127.0.0.1/", "http://localhost/"]
    ...
    for (const [paramName] of params) {
      for (const payload of ssrfPayloads.slice(0, 2)) {
        ...
        if (response && response.status === 200) {
          const text = await response.text()
          if (text.includes("root:") || text.includes("daemon:") || text.length > 1000) {
            this.vulnerabilities.push({
              id: `A10-${Date.now()}`,
